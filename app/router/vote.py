...
def vote(vote: schemas.Vote, db: Session = Depends(get_db), 
    ...
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Post {vote.post_id} not exist")
    ...
    if (vote.dir == 1):
        ...
        if not found_vote:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Vote has not exist")
